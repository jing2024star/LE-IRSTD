...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        if self.pil:  # use PIL
            ...
            try:
                ...
                size = font_size or max(round(sum(self.im.size) / 12 * 0.035), 12)
                self.font = ImageFont.truetype(str(font), size)
            except Exception:
                self.font = ImageFont.load_default()
        else:  # use cv2
            self.im = im
        self.lw = line_width or max(round(sum(im.shape) / 12 * 0.003), 1)  # line width
        # Pose
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                tf = max(1, 0)  # font thickness
                w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 6, thickness=1)[0]  # text width, height
                outside = p1[1] - h >= 3
    ...
    for f in files:
        try:
            ...
            for i, j in enumerate(index):
                ...
                ax[i].set_title(s[j], fontsize=10)
